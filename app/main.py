"""
FastAPI Application — COPAN — Classification-Oriented Phishing Analysis Network.
...
Environment variables:
  ...
  EXPOSE_RAW_FEATURES  — "true" to include raw feature dict in analyze responses
                         (default false)
  ANTHROPIC_API_KEY    — pre-configure the Claude API key so the UI chat works
                         without users entering a key manually
...
# ── FastAPI app ──────────────────────────────────────────────────────────────
app = FastAPI(
    title="COPAN — Classification-Oriented Phishing Analysis Network",
...
async def health_check():
    ...
    return {
        "status":    "healthy",
        "service":   "COPAN — Classification-Oriented Phishing Analysis Network",
        "version":   "1.0.0",
        "timestamp": datetime.utcnow().isoformat() + "Z",
    }
...
async def get_config():
    """Return public configuration flags (does not expose secrets)."""
    return {
        "anthropic_key_configured": bool(os.environ.get("ANTHROPIC_API_KEY", "").strip()),
    }
...
async def chat(request: Request, data: ChatRequest, x_api_key: Optional[str] = Header(None)):
    ...
    # Priority: user-supplied header → ANTHROPIC_API_KEY env var
    api_key = (x_api_key or "").strip() or os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if not api_key:
        raise HTTPException(
            status_code=400,
            detail="No API key available. Set ANTHROPIC_API_KEY on the server or enter your key in the chat settings.",
        )
    ...
    try:
        ...
        system_prompt = (
            ...
            "You are embedded in COPAN (Classification-Oriented Phishing Analysis Network) built on a dual-engine "
